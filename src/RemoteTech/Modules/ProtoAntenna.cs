...
namespace RemoteTech.Modules
{
    internal class ProtoAntenna : IAntenna
    {
        ...
        public bool Connected
        {
            get
            {
                return RTCore.Instance.Network.Graph[Guid].Any(l => l.Interfaces.Contains(this));
            }
        }
        ...
        public Guid Target
        {
            ...
            set
            {
                ...
                if (mProtoModule != null)
                {
                    ...
                    if (i != -1)
                    {
                        mProtoPart.modules[i] = new ProtoPartModuleSnapshot(
                            mProtoModule.moduleValues
                        );
                    }
                }
            }
        ...
        public ProtoAntenna(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot ppms)
        {
            ...
            float temp_float;
            bool temp_bool;
            Dish = Single.TryParse(ppms.moduleValues.GetValue("RTDishRange"), out temp_float)
                ? temp_float
                : 0.0f;
            CosAngle = Double.TryParse(
                ppms.moduleValues.GetValue("RTDishCosAngle"),
                out temp_double
            )
                ? temp_double
                : 0.0;
            Omni = Single.TryParse(ppms.moduleValues.GetValue("RTOmniRange"), out temp_float)
                ? temp_float
                : 0.0f;
            Powered = Boolean.TryParse(ppms.moduleValues.GetValue("IsRTPowered"), out temp_bool)
                ? temp_bool
                : false;
            Activated = Boolean.TryParse(ppms.moduleValues.GetValue("IsRTActive"), out temp_bool)
                ? temp_bool
                : false;
        ...
        public override string ToString()
        {
            return String.Format(
                "ProtoAntenna(Name: {0}, Guid: {1}, Dish: {2}, Omni: {3}, Target: {4}, CosAngle: {5})",
                Name,
                Guid,
                Dish,
                Omni,
