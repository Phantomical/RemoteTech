...
namespace RemoteTech.RangeModel
{
    public static class AbstractRangeModel
    {
        ...
        )
        {
            ...
            double maxDish = Math.Max(
                CheckRange(rangeFunc, antenna.Dish, dishClamp, maxOmniB + bonusB, omniClamp),
                CheckRange(rangeFunc, antenna.Dish, dishClamp, maxDishB, dishClamp)
            );

            if (Double.IsNaN(maxOmni))
            {
                maxOmni = 0.0;
            }
            if (Double.IsNaN(maxDish))
            {
                maxDish = 0.0;
            }
        ...
        /// <param name="rangeFunc">A function that computes the maximum range between two
        /// satellites, given their individual ranges.</param>
        public static NetworkLink<ISatellite> GetLink(
            ISatellite satA,
            ISatellite satB,
            Func<double, double, double> rangeFunc
        )
        {
            ...
            foreach (var ant in omnisA)
            {
                if (
                    ...
                    || CheckRange(rangeFunc, ant.Omni + bonusA, omniClamp, maxDishB, dishClamp)
                        >= distance
                )
                    As.Add(ant);
            }
            ...
            {
                if (
                    CheckRange(rangeFunc, ant.Dish, dishClamp, maxOmniB + bonusB, omniClamp)
                        >= distance
            ...
            foreach (var ant in omnisB)
            {
                if (
                    ...
                    || CheckRange(rangeFunc, ant.Omni + bonusB, omniClamp, maxDishA, dishClamp)
                        >= distance
                )
                    Bs.Add(ant);
            }
            ...
            {
                if (
                    CheckRange(rangeFunc, ant.Dish, dishClamp, maxOmniA + bonusA, omniClamp)
                        >= distance
            ...
            if (selectedAntennaA != null && selectedAntennaB != null)
            {
                ...
                LinkType type = (
                    dishesA.Contains(selectedAntennaA) || dishesB.Contains(selectedAntennaB)
                        ? LinkType.Dish
                        : LinkType.Omni
                );
        ...
        private static double CheckRange(
            Func<double, double, double> rangeFunc,
            double range1,
            double clamp1,
            double range2,
        ...
        {
            return Math.Min(
                Math.Min(rangeFunc.Invoke(range1, range2), range1 * clamp1),
                range2 * clamp2
            );
        }
        ...
        private static double GetMultipleAntennaBonus(
            IEnumerable<IAntenna> omniList,
            double maxOmni
        )
        {
