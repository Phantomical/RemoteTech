...
namespace RemoteTech.SimpleTypes
{
    public class NetworkRoute<T> : IComparable<NetworkRoute<T>>
    {
        public T Goal
        {
            get { return Exists ? Links[Links.Count - 1].Target : default(T); }
        }
        ...
        public double Delay
        {
            get
            {
                return RTSettings.Instance.EnableSignalDelay
        ...
        public bool Contains(BidirectionalEdge<T> edge)
        {
            if (Links.Count == 0)
                return false;
            if (
                (Start.Equals(edge.A) && Links[0].Target.Equals(edge.B))
