...
namespace RemoteTech
{
    public static class NetworkPathfinder
    {
        ...
        public static NetworkRoute<T> Solve<T>(
            T start,
            T goal,
            ...
            where T : class
        {
            ...
            var nStart = new Node<NetworkLink<T>>(
                new NetworkLink<T>(start, null, LinkType.None),
                0,
                heuristicFunction.Invoke(start, goal),
                null,
                false
            );
                ...
                foreach (var link in neighborsFunction.Invoke(current.Item.Target))
                {
                    ...
                    if (nodeMap.ContainsKey(link.Target))
                    {
                        ...
                        if (new_cost <= n.Cost)
                        {
                            ...
                            var new_node = new Node<NetworkLink<T>>(
                                n.Item,
                                new_cost,
                                n.Heuristic,
                                current,
                    ...
                    else
                    {
                        ...
                        var new_node = new Node<NetworkLink<T>>(
                            link,
                            new_cost,
                            heuristicFunction.Invoke(link.Target, goal),
                            current,
                            false
                        );
                        priorityQueue.Enqueue(new_node);
