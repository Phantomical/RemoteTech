...
namespace RemoteTech.UI
{
    ...
    public class ManeuverNodeOverlay : IFragment, IDisposable
    {
        ...
        {
            get { return MapView.fetch; }
        }

        public ManeuverNodeOverlay()
        {
            mManeuverNodeButtonAdd = GUITextureButtonFactory.CreateFromFilename(
                "buttons_fc_add",
                "buttons_fc_add_hover",
                "buttons_fc_add_hover",
                "buttons_fc_add_hover"
            );
            ...
            mManeuverNodeButtonDelete = GUITextureButtonFactory.CreateFromFilename(
                "buttons_fc_del",
                "buttons_fc_del_hover",
                "buttons_fc_del",
                "buttons_fc_del"
            );
            ...
            if (this.mMap != null && FlightGlobals.ActiveVessel != null)
            {
                ...
                if (pCS != null && pCS.maneuverNodes.Count > 0)
                {
                    ...
                    for (var i = 0; i < pCS.maneuverNodes.Count; i++)
                    {
                        ...
                        Vector3 screenCoord = gizmo.camera.WorldToScreenPoint(
                        ...
                        Rect screenPos = new Rect(
                            screenCoord.x - btnWidth - 5.0f,
                            Screen.height - screenCoord.y - btnWidth,
                            btnWidth,
                            btnWidth
                        );
                        ...
                        if (GUILayout.Button("", maneuverCtrl))
                        {
                            if (!nodeAlreadyQueued)
                            {
                                flightComputer.Enqueue(
                                    ManeuverCommand.WithNode(i, flightComputer),
                                    false,
                                    false,
                                    true
                                );
