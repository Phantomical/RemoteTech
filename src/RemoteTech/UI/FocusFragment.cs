        ...
        public void Draw()
        {
            mScrollPosition = GUILayout.BeginScrollView(mScrollPosition, AbstractWindow.Frame);
            {
                ...
                foreach (VesselSatellite sat in RTCore.Instance.Satellites)
                {
                    if (
                        (
                            sat.parentVessel != null
                            && !MapViewFiltering.CheckAgainstFilter(sat.parentVessel)
                        )
                        || FlightGlobals.ActiveVessel == sat.parentVessel
                    )
                    {
                        continue;
                    }
                    ...
                    RTUtil.StateButton(
                        text,
                        mSelection == sat.parentVessel ? 1 : 0,
                        1,
                        s =>
                        {
                            ...
                            if (mSelection != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    ScaledMovement scaledMovement =
                                        new GameObject().AddComponent<ScaledMovement>();
                                    scaledMovement.tgtRef = vessel.transform;
                                    scaledMovement.name = sat.Name;
                                    scaledMovement.transform.parent = ScaledSpace
                                        .Instance
                                        .transform;
                                    scaledMovement.vessel = vessel;
                                    scaledMovement.type = MapObject.ObjectType.Vessel;

                                    var success = PlanetariumCamera.fetch.SetTarget(
                                        PlanetariumCamera.fetch.AddTarget(scaledMovement)
                                    );
                                    ...
                                    if (mLastVessel != vessel)
                                    {
                                        ...
                                        vessel.orbitRenderer.drawIcons = OrbitRenderer
                                            .DrawIcons
                                            .OBJ;
                                        mLastVessel = vessel;
                                    }
