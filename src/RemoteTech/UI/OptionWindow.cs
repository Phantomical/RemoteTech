...
namespace RemoteTech.UI
{
    class OptionWindow : AbstractWindow
    {
        ...
        {
            get { return RTSettings.Instance; }
        }
        ...
        public OptionWindow()
            : base(
                new Guid("387AEB5A-D29C-485B-B96F-CA575E776940"),
                Localizer.Format("#RT_OptionWindow_title", RTUtil.Version), //"RemoteTech " +  + " Options"
                new Rect(
                    Screen.width / 2 - (OptionWindow.WINDOW_WIDTH / 2),
                    Screen.height / 2 - (OptionWindow.WINDOW_HEIGHT / 2),
                    OptionWindow.WINDOW_WIDTH,
                    OptionWindow.WINDOW_HEIGHT
                ),
        ...
        public override void Window(int uid)
        {
            ...
            GUILayout.BeginVertical(
                GUILayout.Width(OptionWindow.WINDOW_WIDTH),
                GUILayout.Height(OptionWindow.WINDOW_HEIGHT)
            );
            {
                // Header image
                GUI.DrawTexture(
                    new Rect(16, 25, OptionWindow.WINDOW_WIDTH - 14, 70),
                    this.mTexHeadline
                );
                GUILayout.Space(70);
        ...
        private void initalAssets()
        {
            ...
            this.mGuiListText = new GUIStyle(HighLogic.Skin.label) { fontSize = 12 };
            ...
            this.loadColorTexture(
                out this.mVSColorRemoteStation,
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                foreach (OPTION_MENUS menu in Enum.GetValues(typeof(OPTION_MENUS)))
                {
                    RTUtil.FakeStateButton(
                        new GUIContent(menu.ToString()),
                        () => this.mMenuValue = (int)menu,
                        this.mMenuValue,
                        (int)menu,
                        GUILayout.Height(16)
                    );
        ...
        private void drawWorldScaleContent()
        {
            GUILayout.Label(
                Localizer.Format(
                    "#RT_OptionWindow_WorldScale_ConsMulti_head",
                    this.mSettings.ConsumptionMultiplier
                ),
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.75f)
            ); //"Consumption Multiplier: (" +  + ")"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_WorldScale_ConsMulti_text"),
                this.mGuiHintText
            ); //"If set to a value other than 1, the power consumption of all antennas will be increased or decreased by this factor.\nDoes not affect energy consumption for science transmissions."
            this.mSettings.ConsumptionMultiplier = (float)
                Math.Round(
                    GUILayout.HorizontalSlider(this.mSettings.ConsumptionMultiplier, 0, 2),
                    2
                );
            ...
            GUILayout.Label(
                Localizer.Format(
                    "#RT_OptionWindow_WorldScale_RangeMulti_head",
                    this.mSettings.RangeMultiplier
                ),
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.75f)
            ); //"Antennas Range Multiplier: (" +  + ")"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_WorldScale_RangeMulti_text"),
                this.mGuiHintText
            ); //"If set to a value other than 1, the range of all <b><color=#bada55>antennas</color></b> will be increased or decreased by this factor.\nDoes not affect Mission Control range."
            mSettings.RangeMultiplier = (float)
                Math.Round(GUILayout.HorizontalSlider(mSettings.RangeMultiplier, 0, 10), 2);

            GUILayout.Label(
                Localizer.Format(
                    "#RT_OptionWindow_WorldScale_MissionControlRangeMulti_head",
                    mSettings.MissionControlRangeMultiplier
                ),
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.75f)
            ); //"Mission Control Range Multiplier: (" +  + ")"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_WorldScale_MissionControlRangeMulti_text"),
                this.mGuiHintText
            ); //"If set to a value other than 1, the range of all <b><color=#bada55>Mission Controls</color></b> will be increased or decreased by this factor.\nDoes not affect antennas range."
            mSettings.MissionControlRangeMultiplier = (float)
                Math.Round(
                    GUILayout.HorizontalSlider(mSettings.MissionControlRangeMultiplier, 0, 10),
                    2
                );
        }
        ...
        {
            GUILayout.Space(10);
            this.mSettings.EnableSignalDelay = GUILayout.Toggle(
                this.mSettings.EnableSignalDelay,
                (this.mSettings.EnableSignalDelay)
                    ? Localizer.Format("#RT_OptionWindow_AlternativeRules_EnableSignalDelay")
                    : Localizer.Format("#RT_OptionWindow_AlternativeRules_disabledSignalDelay")
            ); //"Signal delay enabled""Signal delay disabled"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_AlternativeRules_SignalDelaydesc"),
                this.mGuiHintText
            ...
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_AlternativeRules_RangeModelMode_text"),
                this.mGuiHintText
            ...
            GUILayout.BeginHorizontal();
            {
                RTUtil.FakeStateButton(
                    new GUIContent(
                        Localizer.Format(
                            "#RT_OptionWindow_AlternativeRules_RangeModelMode_Standard"
                        )
                    ),
                    () => this.mSettings.RangeModelType = RangeModel.RangeModel.Standard,
                    (int)this.mSettings.RangeModelType,
                    (int)RangeModel.RangeModel.Standard,
                    GUILayout.Height(20)
                ); //"Standard"
                RTUtil.FakeStateButton(
                    new GUIContent(
                        Localizer.Format("#RT_OptionWindow_AlternativeRules_RangeModelMode_Root")
                    ),
                    () => this.mSettings.RangeModelType = RangeModel.RangeModel.Additive,
                    (int)this.mSettings.RangeModelType,
                    (int)RangeModel.RangeModel.Additive,
                    GUILayout.Height(20)
                ); //"Root"
            }
            GUILayout.EndHorizontal();
            ...
            GUILayout.Label(
                Localizer.Format(
                    "#RT_OptionWindow_AlternativeRules_MultipleAntennaMulti_head",
                    this.mSettings.MultipleAntennaMultiplier
                ),
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.75f)
            ); //"Multiple Antenna Multiplier : (" +  + ")"
            GUILayout.Label(
            ...
            this.mSettings.MultipleAntennaMultiplier = Math.Round(
                GUILayout.HorizontalSlider((float)mSettings.MultipleAntennaMultiplier, 0, 1),
                2
            );
        }
        ...
        private void drawVisualStyleContent()
        {
            ...
            if (this.dishSlider)
            {
                this.mSettings.DishConnectionColor = this.drawColorSlider(
                    this.mSettings.DishConnectionColor
                );
                this.loadColorTexture(out this.mVSColorDish, this.mSettings.DishConnectionColor);
            }
            ...
            {
                GUILayout.Label(
                    Localizer.Format("#RT_OptionWindow_VisualStyle_OmniColor"),
                    GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.75f)
                ); //"Omni Connection Color:"
            ...
            if (this.omniSlider)
            {
                this.mSettings.OmniConnectionColor = this.drawColorSlider(
                    this.mSettings.OmniConnectionColor
                );
                this.loadColorTexture(out this.mVSColorOmni, this.mSettings.OmniConnectionColor);
            }
            ...
            {
                GUILayout.Label(
                    Localizer.Format("#RT_OptionWindow_VisualStyle_ActiveColor"),
                    GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.75f)
            ...
            {
                this.mSettings.ActiveConnectionColor = this.drawColorSlider(
                    this.mSettings.ActiveConnectionColor
                );
                this.loadColorTexture(
                    out this.mVSColorActive,
                    this.mSettings.ActiveConnectionColor
                );
            }
            ...
            {
                this.mSettings.RemoteStationColorDot = this.drawColorSlider(
                    this.mSettings.RemoteStationColorDot
                );
                this.loadColorTexture(
                    out this.mVSColorRemoteStation,
            ...
            this.mSettings.HideGroundStationsBehindBody = GUILayout.Toggle(
                this.mSettings.HideGroundStationsBehindBody,
                (this.mSettings.HideGroundStationsBehindBody)
                    ? Localizer.Format("#RT_OptionWindow_VisualStyle_StationsBehindBodyHide")
                    : Localizer.Format("#RT_OptionWindow_VisualStyle_StationsBehindBodyShow")
            ); //"Ground Stations are hidden behind bodies""Ground Stations always shown"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_VisualStyle_StationsBehindBodytext"),
                this.mGuiHintText
            ...
            this.mSettings.HideGroundStationsOnDistance = GUILayout.Toggle(
                this.mSettings.HideGroundStationsOnDistance,
                (this.mSettings.HideGroundStationsOnDistance)
                    ? Localizer.Format("#RT_OptionWindow_VisualStyle_StationsOnDistancehide")
                    : Localizer.Format("#RT_OptionWindow_VisualStyle_StationsOnDistanceShow")
            ); //"Ground Stations are hidden at a defined distance""Ground Stations always shown"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_VisualStyle_StationsOnDistanceText"),
                this.mGuiHintText
            ...
            this.mSettings.ShowMouseOverInfoGroundStations = GUILayout.Toggle(
                this.mSettings.ShowMouseOverInfoGroundStations,
                (this.mSettings.ShowMouseOverInfoGroundStations)
                    ? Localizer.Format("#RT_OptionWindow_VisualStyle_MouseOverInfoGroundStationsE")
                    : Localizer.Format("#RT_OptionWindow_VisualStyle_MouseOverInfoGroundStationsD")
            ); //"Mouseover of Ground Stations enabled""Mouseover of Ground Stations disabled"
            GUILayout.Label(
        ...
        {
            GUILayout.Space(10);
            this.mSettings.ThrottleTimeWarp = GUILayout.Toggle(
                this.mSettings.ThrottleTimeWarp,
                (this.mSettings.ThrottleTimeWarp)
                    ? Localizer.Format("#RT_OptionWindow_Miscellaneous_ThrottleTimeWarp")
                    : Localizer.Format("#RT_OptionWindow_Miscellaneous_NoThrottleTimeWarp")
            ); //"RemoteTech will throttle time warp""RemoteTech will not throttle time warp"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_Miscellaneous_ThrottleTimeWarp_text"),
                this.mGuiHintText
            ); //"ON: The flight computer will automatically stop time warp a few seconds before executing a queued command.\nOFF: The player is responsible for controlling time warp during scheduled actions."

            this.mSettings.ThrottleZeroOnNoConnection = GUILayout.Toggle(
                this.mSettings.ThrottleZeroOnNoConnection,
                (this.mSettings.ThrottleZeroOnNoConnection)
                    ? Localizer.Format("#RT_OptionWindow_Miscellaneous_ThrottleZeroOnNoConnection")
                    : Localizer.Format(
                        "#RT_OptionWindow_Miscellaneous_NoThrottleZeroOnNoConnection"
                    )
            ); //"Throttle to zero on loss of connection""Throttle unaffected by loss of connection"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_Miscellaneous_ThrottleZeroOnNoConnection_text"),
                this.mGuiHintText
            ); //"ON: The flight computer cuts the thrust if you lose connection to Mission Control.\nOFF: The throttle is not adjusted automatically."

            this.mSettings.StopTimeWrapOnReConnection = GUILayout.Toggle(
                this.mSettings.StopTimeWrapOnReConnection,
                (this.mSettings.StopTimeWrapOnReConnection)
                    ? Localizer.Format("#RT_OptionWindow_Miscellaneous_StopTimeWrapOnReConnection")
                    : Localizer.Format(
                        "#RT_OptionWindow_Miscellaneous_NoStopTimeWrapOnReConnection"
                    )
            ); //"Stop time wrap on reconnection""Time wrap uninterrupted by reconnection"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_Miscellaneous_StopTimeWrapOnReConnection_text"),
                this.mGuiHintText
            ...
            this.mSettings.UpgradeableMissionControlAntennas = GUILayout.Toggle(
                this.mSettings.UpgradeableMissionControlAntennas,
                (this.mSettings.UpgradeableMissionControlAntennas)
                    ? Localizer.Format("#RT_OptionWindow_Miscellaneous_UpgradeableMissionControl")
                    : Localizer.Format("#RT_OptionWindow_Miscellaneous_NoUpgradeableMissionControl")
            ); //"Mission Control antennas are upgradeable""Mission Control antennas are not upgradeable"
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_Miscellaneous_UpgradeableMissionControlText"),
                this.mGuiHintText
            ); //"ON: Mission Control antenna range is upgraded when the Tracking Center is upgraded.\nOFF: Mission Control antenna range isn't upgradeable."

            this.mSettings.AutoInsertKaCAlerts = GUILayout.Toggle(
                this.mSettings.AutoInsertKaCAlerts,
                (this.mSettings.AutoInsertKaCAlerts)
                    ? Localizer.Format("#RT_OptionWindow_Miscellaneous_AutoInsertKaCAlerts")
        ...
        {
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_Presets_HelpText"),
                this.mGuiRunningText
            ); //"You can revert your current settings to the starting settings, constructed from installed mods' MM patches. Also, you can reload your current settings with a third-party mod's own RemoteTech settings (the fallback in the event of no MM patch).\n\nHere you can see what presets are available:"
            GUILayout.Space(15);
            ...
            {
                GUILayout.Label(
                    Localizer.Format("#RT_OptionWindow_Presets_NotFound"),
                    this.mGuiRunningText
            ...
            for (int i = presetList.Count - 1; i >= 0; --i)
            {
                GUILayout.BeginHorizontal("box", GUILayout.MaxHeight(15));
                {
                    ...
                    folderName =
                        folderName.Substring(0, index)
                        + folderName.Substring(index).Replace("/RemoteTechSettings", "").Trim();
                    ...
                    if (index >= 0)
                        folderName =
                            ...
                            + folderName
                                .Substring(index)
                                .Replace("/Default_Settings", "starting settings");

                    GUILayout.Space(15);
                    GUILayout.Label(folderName, this.mGuiListText, GUILayout.ExpandWidth(true));
                    if (
                        GUILayout.Button(
                            Localizer.Format("#RT_OptionWindow_Presets_Reload"),
                            this.mGuiListButton,
                            GUILayout.Width(70),
                            GUILayout.Height(20)
                        )
                    ) //"Reload"
                    {
                        ...
                        ScreenMessages.PostScreenMessage(
                            Localizer.Format("#RT_OptionWindow_Presets_msg1", folderName),
                            10
                        ); //string.Format("Your RemoteTech settings are set to {0}", )
                        RTLog.Notify(
                            "Overwrote current settings with this cfg {0}",
                            RTLogLevel.LVL3,
                            presetList[i]
                        );
                    }
        ...
        {
            GUILayout.Space(10);
            this.mSettings.ControlAntennaWithoutConnection = GUILayout.Toggle(
                this.mSettings.ControlAntennaWithoutConnection,
                (this.mSettings.ControlAntennaWithoutConnection)
                    ? Localizer.Format("#RT_OptionWindow_Cheat_ControlAntennaWithoutConnection")
                    : Localizer.Format("#RT_OptionWindow_Cheat_ControlAntennaNeedConnection")
            ...
            GUILayout.Space(10);
            this.mSettings.IgnoreLineOfSight = GUILayout.Toggle(
                this.mSettings.IgnoreLineOfSight,
                (this.mSettings.IgnoreLineOfSight)
                    ? Localizer.Format("#RT_OptionWindow_Cheat_IgnoreLineOfSight")
                    : Localizer.Format("#RT_OptionWindow_Cheat_NoIgnoreLineOfSight")
            ); //"Planets and moons will not block a signal""Planets and moons will block a signal"
            GUILayout.Label(
        ...
        private Color drawColorSlider(Color value)
        {
            ...
            GUILayout.BeginVertical();
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_ColorSlider_Red", (int)(value.r * 255)),
                this.mGuiHintText,
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.25f)
            ); //"Red: ("+ +")"
            value.r = GUILayout.HorizontalSlider(value.r, 0, 1);
            GUILayout.EndVertical();
            GUILayout.BeginVertical();
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_ColorSlider_Green", (int)(value.g * 255)),
                this.mGuiHintText,
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.25f)
            ); //"Green: (" +  + ")"
            ...
            GUILayout.BeginVertical();
            GUILayout.Label(
                Localizer.Format("#RT_OptionWindow_ColorSlider_Blue", (int)(value.b * 255)),
                this.mGuiHintText,
                GUILayout.Width(OptionWindow.WINDOW_WIDTH * 0.25f)
            ); //"Blue: (" +  + ")"
