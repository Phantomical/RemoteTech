...
namespace RemoteTech.UI
{
    public class RoverFragment : IFragment
    {
        ...
        private readonly GUIContent[] Tabs =
        {
            new GUIContent(
                Localizer.Format("#RT_RoverFragment_TGT"),
                Localizer.Format("#RT_RoverFragment_TGT_desc")
            ), //"TGT", "Drive to the latitude and longitude of a body or towards vessel target."
            new GUIContent(
                Localizer.Format("#RT_RoverFragment_HDG"),
                Localizer.Format("#RT_RoverFragment_HDG_desc")
            ), //"HDG", "Drive with specific heading and distance."
            new GUIContent(
                Localizer.Format("#RT_RoverFragment_FINE"),
                Localizer.Format("#RT_RoverFragment_FINE_desc")
            ),
        }; //"FINE", "Drive with specific turning or distance."

        private enum RoverModes
        {
            TargetMode = 0,
            HeadingMode = 1,
            FineMode = 2,
        ...
        public void Draw()
        {
            ...
            {
                selectedModeIndex = GUILayout.Toolbar(
                    selectedModeIndex,
                    Tabs,
                    GUILayout.Width(width3 * 3 + GUI.skin.button.margin.right * 2.0f)
                );
                ...
                GUILayout.BeginHorizontal();
                {
                    RTUtil.Button(
                        new GUIContent(
                            Localizer.Format("#RT_RoverFragment_DRIVE"),
                            Localizer.Format("#RT_RoverFragment_DRIVE_desc")
                        ), //"DRIVE", "Starts the automatic driving."
                        delegate
                        {
                            OnExecClick(selectedModeIndex);
                        },
                        GUILayout.Width(width3)
                    );
                    GUILayout.FlexibleSpace();
                    RTUtil.Button(
                        new GUIContent(">>", Localizer.Format("#RT_RoverFragment_Queue_desc")), //"Toggles the queue and delay functionality."
                        mOnClickQueue,
                        GUILayout.Width(width3)
                    );
                }
        ...
        private void OnExecClick(int modeIndex)
        {
            ...
            else if (modeIndex == (int)RoverModes.HeadingMode)
            {
                mFlightComputer.Enqueue(
                    DriveCommand.DistanceHeading(Dist, Heading, mSteerClamp, Speed)
                );
            }
            //TGT
            ...
            {
                mFlightComputer.Enqueue(
                    DriveCommand.Coord(mSteerClamp, Latitude, Longitude, Speed)
                );
            }
        }
        ...
        private void DrawFineContent()
        {
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_FINESteer"),
                        Localizer.Format("#RT_RoverFragment_FINESteer_desc")
                    )
                ); //"Steer: ", "How much to turn"
                GUILayout.FlexibleSpace();
                ...
                if (mSteering != 0)
                {
                    if (mSteering < 0)
                        GUILayout.Label(
                            new GUIContent(Localizer.Format("#RT_RoverFragment_right"), ""),
                            GUILayout.Width(40)
                        ); //"right"
                    else
                        GUILayout.Label(
                            new GUIContent(Localizer.Format("#RT_RoverFragment_left"), ""),
                            GUILayout.Width(40)
                        ); //"left"
                }
                else
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_Turn"),
                        Localizer.Format("#RT_RoverFragment_Turn_desc")
                    ),
                    GUILayout.Width(50)
                ); //"Turn", "How many degrees to turn"
                GUI.SetNextControlName("RC1");
                RTUtil.TextField(ref mTurn, GUILayout.Width(50), GUILayout.ExpandWidth(false));
                GUILayout.Label(
                    new GUIContent("(°)", Localizer.Format("#RT_RoverFragment_Turn_desc")),
                    GUI.skin.textField,
                    GUILayout.Width(40)
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_Dist"),
                        Localizer.Format("#RT_RoverFragment_Dist_desc")
                    ),
                    GUILayout.Width(50)
                ); //"Dist", "Distance to drive"
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_Speed"),
                        Localizer.Format("#RT_RoverFragment_Speed_desc")
                    ),
                    GUILayout.Width(50)
                ); //"Speed", "Speed to maintain, negative for reverse"
        ...
        private void DrawHDGContent()
        {
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_HDGSteer"),
                        Localizer.Format("#RT_RoverFragment_HDGSteer_desc")
                    )
                ); //"Steer: ", "How sharp to turn at max"
                GUILayout.FlexibleSpace();
                GUILayout.Label(
                    new GUIContent(
                        mSteerClamp.ToString("P"),
                        Localizer.Format("#RT_RoverFragment_HDGSteer_desc")
                    )
                ); //"How sharp to turn at max"
                GUILayout.Label(
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_mHdg"),
                        Localizer.Format("#RT_RoverFragment_mHdg_desc")
                    ),
                    GUILayout.Width(50)
                ); //"Hdg", "Heading to maintain"
                GUI.SetNextControlName("RC1");
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_Speed"),
                        Localizer.Format("#RT_RoverFragment_HDGSpeed_desc")
                    ),
                    GUILayout.Width(50)
                ...
                RTUtil.TextField(ref mSpeed, GUILayout.Width(50), GUILayout.ExpandWidth(false));
                GUILayout.Label(
                    new GUIContent("(m/s)", Localizer.Format("")),
                    GUI.skin.textField,
                    GUILayout.Width(40)
                ); //"Speed to maintain"
            }
        ...
        private void DrawTargetContent()
        {
            ...
            string tooltip = Localizer.Format(
                "#RT_RoverFragment_coordinations_desc",
                GameSettings.MODIFIER_KEY.name
            ); //"Hold " +  + " and click on ground to input coordinates"
            ITargetable Target = mFlightComputer.Vessel.targetObject;

            if (
                GameSettings.MODIFIER_KEY.GetKey()
                && ((Input.GetMouseButton(0) || Input.GetMouseButton(1)) != MouseClick)
            ) // on lookout for mouse click on body
            {
            ...
            GUILayout.Label(
                new GUIContent(
                    Localizer.Format("#RT_RoverFragment_ModeLabel", targetTypeString),
                    tooltip
                )
            ); //"Mode: "
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_LAT"),
                        Localizer.Format("#RT_RoverFragment_LAT_desc")
                    ),
                    GUILayout.Width(50)
                ); //"LAT", "Latitude to drive to"
                GUI.SetNextControlName("RC1");
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(
                    new GUIContent(
                        Localizer.Format("#RT_RoverFragment_LON"),
                        Localizer.Format("#RT_RoverFragment_LON_desc")
                    ),
                    GUILayout.Width(50)
                ); //"LON", "Longitude to drive to"
                GUI.SetNextControlName("RC2");
                RTUtil.TextField(
                    ref mLongditude,
                    GUILayout.Width(50),
