...
namespace RemoteTech.UI
{
    ...
    public class TimeWarpDecorator
    {
        ...
        /// Activ Vessel
        /// </summary>
        private VesselSatellite mVessel
        {
            get { return RTCore.Instance.Satellites[FlightGlobals.ActiveVessel]; }
        }
                ...
                else if (vs.Connections.Any())
                {
                    if (RTSettings.Instance.EnableSignalDelay)
                    {
                        return Localizer.Format(
                            "#RT_ConnectionStatus3",
                            vs.Connections[0].Delay.ToString("F5")
                        ); //"D+ " +  + "s"
                    }
                    else
                    {
        ...
        {
            mFlightButtonGreen = GUITextureButtonFactory.CreateFromFilename(
                "texFlightGreen",
                "texFlightGreenOver",
                "texFlightGreenDown",
                "texFlightGreenOver"
            );
            mFlightButtonYellow = GUITextureButtonFactory.CreateFromFilename(
                "texFlightYellow",
                "texFlightYellowOver",
                "texFlightYellowDown",
                "texFlightYellowOver"
            );
            mFlightButtonRed = GUITextureButtonFactory.CreateFromFilename(
                "texFlightRed",
                "texFlightRed",
                "texFlightRed",
                "texFlightRed"
        ...
        public void Draw()
        {
            ...
            if (
                ActionGroupsFlightController.Instance != null
                && ActionGroupsFlightController.Instance.IsOpen
            )
                return;
            ...
            if (
                ...
                && EVAConstructionModeController.Instance.IsOpen
            )
                return;

            Vector2 timeWarpImageScreenCoord = UIMainCamera.Camera.WorldToScreenPoint(
                mTimewarpTransform.position
            );

            float scale = GameSettings.UI_SCALE_TIME * GameSettings.UI_SCALE;
            float YToTimeQuadrant =
                Screen.height
                - (timeWarpImageScreenCoord.y - (mTimewarpTransform.rect.height * scale));
            ...
            Rect delaytextPosition = new Rect(
                (XToTimeQuadrant + 12.0f) * scale,
                YToTimeQuadrant + 2 * scale,
                50.0f * scale,
                20.0f * scale
            );
