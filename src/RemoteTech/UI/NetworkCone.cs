...
namespace RemoteTech.UI
{
    public class NetworkCone : MonoBehaviour
    {
        ...
        public Color Color
        {
            set { mMeshFilter.mesh.colors = Enumerable.Repeat(value, 8).ToArray(); }
        }
        ...
        private void UpdateMesh(Vector3d center, IAntenna dish)
        {
            ...
            Vector3d antennaPos = ScaledSpace.LocalToScaledSpace(
                RTCore.Instance.Network[dish.Guid].Position
            );
            Vector3d planetPos = ScaledSpace.LocalToScaledSpace(center);

            CelestialBody refFrame = (
                MapView.MapCamera.target.vessel != null
            ...
            Vector3 space =
                Vector3.Cross(planetPos - antennaPos, up).normalized
                * Vector3.Distance(antennaPos, planetPos)
                * (float)Math.Tan(Math.Acos(dish.CosAngle));
            Vector3d end1 =
                antennaPos
                + (planetPos + space - antennaPos).normalized
                    * Math.Min(
                        dish.Dish / ScaledSpace.ScaleFactor,
                        Vector3.Distance(antennaPos, planetPos)
                    );
            Vector3d end2 =
                antennaPos
            ...
            var segment1 =
                new Vector3(lineEnd1.y - lineStart.y, lineStart.x - lineEnd1.x, 0).normalized
                * (LineWidth / 2);
            var segment2 =
                new Vector3(lineEnd2.y - lineStart.y, lineStart.x - lineEnd2.x, 0).normalized
                * (LineWidth / 2);
        ...
        private void SetupMesh()
        {
            ...
            mMeshFilter.mesh.uv = new Vector2[8]
            {
                new Vector2(0, 1),
                new Vector2(0, 0),
                new Vector2(1, 1),
            ...
            mMeshFilter.mesh.SetIndices(
                new int[] { 0, 2, 1, 2, 3, 1, 4, 6, 5, 6, 7, 5 },
