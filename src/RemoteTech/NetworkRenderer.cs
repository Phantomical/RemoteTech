...
namespace RemoteTech
{
    ...
    public class NetworkRenderer : MonoBehaviour
    {
        ...
        {
            get { return RTSettings.Instance.MapFilter; }
            set
            {
        ...
        private readonly HashSet<BidirectionalEdge<ISatellite>> mEdges =
            new HashSet<BidirectionalEdge<ISatellite>>();
        ...
        public bool ShowOmni
        {
            get { return (Filter & MapFilter.Omni) == MapFilter.Omni; }
        }
        public bool ShowDish
        {
            get { return (Filter & MapFilter.Dish) == MapFilter.Dish; }
        }
        public bool ShowPath
        {
            get { return (Filter & MapFilter.Path) == MapFilter.Path; }
        }
        public bool ShowMultiPath
        {
            get { return (Filter & MapFilter.MultiPath) == MapFilter.MultiPath; }
        }
        public bool ShowRange
        {
            get { return (Filter & MapFilter.Sphere) == MapFilter.Sphere; }
        }
        public bool ShowCone
        {
            get { return (Filter & MapFilter.Cone) == MapFilter.Cone; }
        }
        ...
        public static NetworkRenderer CreateAndAttach()
        {
            ...
            renderer.smallStationHead = new GUIStyle(HighLogic.Skin.label) { fontSize = 12 };
        ...
        public void OnGUI()
        {
            ...
            {
                foreach (
                    ISatellite s in RTCore
                        .Instance.Satellites.FindCommandStations()
                        .Concat(RTCore.Instance.Network.GroundStations.Values)
                )
                {
                    ...
                    if (
                        RTSettings.Instance.HideGroundStationsBehindBody
                        && IsOccluded(s.Position, s.Body)
                    )
                        showOnMapview = false;

                    if (
                        RTSettings.Instance.HideGroundStationsOnDistance
                        && !IsOccluded(s.Position, s.Body)
                        && this.IsCamDistanceToWide(s.Position)
                    )
                        showOnMapview = false;
                    ...
                    if (showOnMapview)
                    {
                        ...
                        if (
                            RTSettings.Instance.ShowMouseOverInfoGroundStations
                            && s is MissionControlSatellite
                            && screenRect.ContainsMouse()
                        )
                        {
                            Rect headline = screenRect;
                            Vector2 nameDim = this.smallStationHead.CalcSize(
                            ...
                            foreach (var antenna in s.Antennas)
                            {
                                ...
                                {
                                    antennaRanges +=
                                        Localizer.Format("#RT_NetworkFB_Omni")
                                        + RTUtil.FormatSI(antenna.Omni, "m")
                                        + Environment.NewLine; //"Omni: "
                                }
                                if (antenna.Dish > 0)
                                {
                                    antennaRanges +=
                                        Localizer.Format("#RT_NetworkFB_Dish")
                                        + RTUtil.FormatSI(antenna.Dish, "m")
                                        + Environment.NewLine; //"Dish: "
                                }
                            }
                            ...
                            if (!antennaRanges.Equals(String.Empty))
                            {
                                ...
                                float maxHeight = this.smallStationText.CalcHeight(
                                    content,
                                    antennaDim.x
                                );
        ...
        {
            Vector3d camPos = ScaledSpace.ScaledToLocalSpace(
        ...
        private void UpdateNetworkCones()
        {
            List<IAntenna> antennas = (
                ShowCone
                    ? RTCore.Instance.Antennas.Where(ant =>
                        ant.Powered
                        && ant.CanTarget
                        && RTCore.Instance.Satellites[ant.Guid] != null
                        && ant.Target != Guid.Empty
                    )
                    : Enumerable.Empty<IAntenna>()
            ).ToList();
            ...
            mCones.AddRange(
                Enumerable.Repeat((NetworkCone)null, Math.Max(newLength - oldLength, 0))
            );
            ...
            for (int i = 0; i < newLength; i++)
            {
                ...
                Debug.Assert(
                    ...
                    "center != null",
                    String.Format(
                        "GetPositionFromGuid returned a null value for the target {0}",
                        antennas[i].Target
                    )
                );
        ...
        private void UpdateNetworkEdges()
        {
            ...
            mLines.AddRange(
                Enumerable.Repeat<NetworkLine>(null, Math.Max(newLength - oldLength, 0))
            );
        ...
        private Color CheckColor(BidirectionalEdge<ISatellite> edge)
        {
            ...
            if (RTSettings.Instance.SignalRelayEnabled)
            {
                ...
                if (
                    (satA != null && !satA.CanRelaySignal) || (satB != null && !satB.CanRelaySignal)
                )
