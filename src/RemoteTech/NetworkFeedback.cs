...
namespace RemoteTech
{
    public class NetworkFeedback
    {
        ...
        {
            if (RTCore.Instance != null && RTCore.Instance.Network != null && target != Guid.Empty)
            {
        ...
        public static int countInCone(IAntenna antenna, Guid target)
        {
            ...
            try
            {
                ...
                foreach (ISatellite sat in RTCore.Instance.Satellites)
                {
                    ...
                    if (
                        Vector3d.Distance(myPos, sat.Position) <= antenna.Dish // in range
                        && Vector3d.Dot(targetDir, satDir) >= antenna.CosAngle // in cone
                        && sat.Body == myBody
                    ) // in target SoI... remove later
                        count++;
                }
        ...
        public static KeyValuePair<string, UnityEngine.Color> tryConnection(
            IAntenna antenna,
            Guid target
        )
        {
            ...
            if (
                ...
                && target != Guid.Empty
                && target != NetworkManager.ActiveVesselGuid
            )
            {
                bool warning = false,
                    error = false;
                ...
                if (targetSat != null)
                {
                    ...
                    conditions.Add(
                        Localizer.Format("#RT_NetworkFB_Currentdistance")
                            + RTUtil.FormatSI(dist, "m")
                    ); //"Current distance:"
                    conditions.Add(
                        Localizer.Format("#RT_NetworkFB_Antennarange")
                ...
                try
                {
                    ...
                    if (dist <= maxDist)
                    {
                        conditions.Add(
                            Localizer.Format(
                                "#RT_NetworkFB_Targetinfo",
                                RTUtil.FormatSI(spread, "m"),
                                numTargets
                            )
                        ); //String.Format("Info:{0} beam covers {1} targets)", , )
                    }
                    else
                    {
                        status = Localizer.Format("#RT_NetworkFB_status3"); //"Target not in range"
                ...
                return new KeyValuePair<string, UnityEngine.Color>(
                    ...
                    error
                        ...
                        : (warning ? UnityEngine.Color.yellow : UnityEngine.Color.white)
                );
            }
